...
import { Achievement, Application, CustomRole, DeveloperProfile, Education, Experience, SavedRole, Skill, Profile } from '@/types/developer';
    ...
    if (data.experience) {
      ...
      if (Array.isArray(data.experience) && data.experience.length > 0) {
        await prisma.experience.createMany({
          data: data.experience.map((exp: Experience) => ({
            developerId: session.user.id,
            title: exp.title,
            company: exp.company,
            description: exp.description,
    ...
    if (data.education) {
      ...
      if (Array.isArray(data.education) && data.education.length > 0) {
        await prisma.education.createMany({
          data: data.education.map((edu: Education) => ({
            developerId: session.user.id,
            degree: edu.degree || null,
            institution: edu.institution,
            year: edu.year,
