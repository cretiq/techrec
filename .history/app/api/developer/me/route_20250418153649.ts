...
export async function GET() {
  try {
    ...
    const mappedDeveloper: DeveloperProfile = {
      ...
      portfolio: developer.contactInfo?.portfolio || undefined,
      skills: skills,
      achievements: developer.achievements.map(achievement => ({
        id: achievement.id,
        title: achievement.title,
        description: achievement.description,
        date: achievement.date.toISOString().split('T')[0], // Convert Date to string
      ...
      experience: developer.experience.map(exp => ({
        ...
        date: exp.startDate.toISOString().split('T')[0] // Convert Date to string
      })),
      education: developer.education.map(edu => ({
        id: edu.id,
        title: edu.degree,
        description: edu.institution,
        date: edu.startDate.toISOString().split('T')[0] // Convert Date to string
      })),
      applications: developer.applications.map(app => ({
        id: app.id,
        title: app.title,
        description: app.description || '',
        date: app.createdAt.toISOString().split('T')[0] // Convert Date to string
      })),
      savedRoles: developer.savedRoles.map(role => ({
        id: role.id,
        title: role.roleId,
        description: role.notes || '',
        date: role.createdAt.toISOString().split('T')[0] // Convert Date to string
      })),
      totalExperience: undefined // Optional field
    };
...
export async function PUT(request: Request) {
  try {
    ...
    const mappedDeveloper: DeveloperProfile = {
      ...
      portfolio: finalDeveloper.contactInfo?.portfolio || undefined,
