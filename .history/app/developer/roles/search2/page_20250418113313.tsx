...
export default function RolesSearch2Page() {
  ...
  const fetchRoles = async () => {
    ...
    try {
      ...
      console.log(`[RolesSearch2Page] Fetching from ${apiUrl}`)
      ...
      const apiJobs: RapidApiJob[] = data?.jobs || [] 
      const mappedRoles: Role[] = apiJobs.map(mapRapidApiJobToRole)

      if (!Array.isArray(mappedRoles)) {
        console.error("[RolesSearch2Page] Received non-array data after mapping:", mappedRoles)
        throw new Error("Invalid roles data format after mapping")
      }

      const validRoles = mappedRoles.filter(role => {
        const isValid = role && typeof role === 'object' && role.id
        if (!isValid) {
          console.warn("[RolesSearch2Page] Invalid role found after mapping (missing ID?):", role)
        }
        return isValid
      })

      console.log("[RolesSearch2Page] Setting valid mapped roles:", validRoles)
      setRoles(validRoles)
    } catch (error: any) {
