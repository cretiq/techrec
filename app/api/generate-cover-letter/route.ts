...
export async function POST(req: Request) {
  try {
    ...
    const { userInfo, jobInfo, companyInfo, jobSourceInfo, developerSkills } = data
...
Role & Company:
The letter is for the position of ${jobInfo.title} at ${companyInfo.name}.

Recipient:
Address the letter to ${companyInfo.name}.
...
Highlight ${userInfo.name}'s proficiency in skills relevant to the job description. Focus on:
${user.map(skill => `- ${skill}`).join('\n')}

Relevant Experience (Tailor to Job Description):
Briefly showcase key ${userInfo.name}'s experiences or projects that directly relate to the requirements of the ${jobInfo.title} role. Quantify achievements where possible.
...
${jobInfo.title} Reason: The specific challenges outlined in the job description for the ${jobInfo.title} role.

Explain why you believe ${userInfo.name}'s skills and experience make you a strong candidate for this specific role (${jobInfo.title}) and how you can contribute to ${companyInfo.name}'s goals.

Tone: Maintain a professional, confident, and enthusiastic tone. Avoid generic statements.

Structure:
- Introduction: State the ${jobInfo.title} position you're applying for and ${jobSourceInfo.source} where you saw it (if applicable). Briefly express enthusiasm.
- Body Paragraph(s): Connect ${userInfo.name} key skills and specific experiences directly to the ${jobInfo.title} requirements. Explain your interest in the ${companyInfo.name} company. Show, don't just tell.
